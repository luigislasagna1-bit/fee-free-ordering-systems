...
**Last updated:** 2026-07-19 by Claude (Erick decision executed on paper: **T-J** = the 3-minute click path for his $10 email-locked credit via the existing VIP-special flow — no new code, nothing sends until you click. Earlier same night: 🎉 **A20 + A21 both CLOSED**: your real test order `ORD-067045266` carried the FIRSTBUY 10% all the way to the charged payment — incident fully resolved; the two delivery indexes pushed to both databases and prod-verified. Remaining opens: T-J clicks, awaiting Apple ×1 + Google ×2 review emails, B5 Kitchen 16 KB real fix.)
...
- **T-J. 🤔 Erick's $10 make-good — ~3 minutes of clicking, nothing sends until YOU click.** Your call from 2026-07-19 ("no comp; email him a $10 credit locked to his email"). No new code needed — the existing **Give a VIP special** flow does exactly this (auto-applies when he uses the same email at checkout; guest, no code to type, no account needed). Steps:
  1. **Admin → Promotions → New promotion** → type **"Fixed Discount on Cart"** → amount **$10**.
  2. On the restrictions step: Display = **"Hidden"** (it will ask for a coupon code — type anything, e.g. `ERICK10`; nobody will ever see or need it, this just keeps the deal OFF your public menu). Turn ON **"Once per customer for life"**. Customer type: leave **everyone** (⚠️ NOT "new customers" — he counts as returning now, that would silently block it — the FIRSTBUY trap again). Optional: an end date (~1 month) and/or a minimum order. Save it **Active**.
  3. **Admin → Customers → open Erick** (the $12.79 order from 2026-07-17) → **"Give a VIP special"** card → pick the new promo → keep **"notify"** checked → **Give**. That click sends him the email (deal name + your description + "use this email at checkout, it applies automatically", in his language-independent branded template) — so write the promo **name/description** as the apology; suggested text is in the chat (Claude has a draft ready).
  4. Done. It auto-applies exactly once on his next order with that email; the trash icon on his customer page removes it any time.
  ⚠️ One flag: $10 on his $12.79 first order ≈ a nearly-free meal — that's your stated intent, just confirming. A minimum order (e.g. $15) is the knob if you want the credit to ride on a bigger order.
