...
*(NOTE: the customer marketplace is already live + free. The FULL admin/pricing/marketing/terms/refund copy sweep + PAYG-route retirement shipped 2026-07-20 (commit bb527957) — no surface still frames the marketplace as paid, and the obsolete pay-as-you-go opt-in pages/routes are retired. **IMPORTANT — A14 is now SAFE to run:** that commit also fixed a Stripe-webhook bug where cancelling a legacy marketplace subscription set `isListed=false`. Before the fix, running A14 (which cancels those subs) would have HIDDEN every legacy subscriber from the now-free marketplace. Now cancelling a legacy sub has no effect on the free listing — those restaurants stay listed.)*
