...
**Last updated:** 2026-08-23 (early) by Claude (**A71 — Every Nabil call is now graded automatically, and there is a Quality page.** Two graders run after each call: a free, rule-based one (dead air, stuck transfers, wrong totals, tool errors, WHY a caller hung up — at the greeting, mid-order, after hearing the total…) and, for real conversations only, an independent AI reviewer (Opus, ≈5¢ a call, ≈$2/day for a busy store — see COSTS.md v6) that scores understanding, accuracy, conversation, grounding and handling and lists concrete issues with the turn number. Superadmin › **Nabil Quality** shows averages by lane (live vs staging) and by version, day by day, why calls end early, the most common findings, and a review queue where the worst calls come first; each call page has the transcript, both verdicts and a Good/Bad/tags review box. I graded the live line's last 72 hours (144 calls): average 83/100; the things dragging it down are exactly what Phase A fixes on staging (47 dead-air turns, 5 stuck transfers) — so once you've made the staging calls and we promote, this page is how you'll SEE the difference, by lane, with numbers. Also fixed in passing: "delete my data" now scrubs the messages callers leave for the store (the new take-a-message feature) and includes them in the data export. **Nothing new needed from you beyond A67 #1 (the staging phone number) and the two yes/no decisions in A70.**)
**Previous update:** 2026-08-22 (night) by Claude (**A70 — Phase A of the call-analysis plan is COMPLETE on the staging copy (11 fixes in 12 commits today); nothing has touched your live number.** Since A69, four more landed: (6) **hearing** — on your ambience line Nabil now gets your menu words as recognition hints (it never did — street names and items like "Veloque"/"cornbread" came from that), a short answer spoken while Nabil is still talking ("Pickup.") is kept instead of thrown away, a one-word "no"/"wait"/"stop" cuts Nabil off, Nabil stops the instant you cut in (no extra second of audio), and a caller repeating themselves or saying "hello?" while Nabil is thinking no longer triggers a second answer; (7) **checkout** — "anything else?" is asked once, a "no" to the total leads to "what would you like to change?" instead of the loop, a "yes" to the wrong question never places an order, and **Nabil now hangs up itself** after one goodbye (no more three goodbyes and a minute of dead line); (8) **robocalls** — an automated menu on the line (the DoorDash "press one if your store is open" call) is recognised from its first sentence, logged as spam and hung up on, with no AI turn spent. **Two small decisions for you, no rush: ☐ (a) should robocall seconds count toward your Nabil minutes? Today they do (the meter deliberately counts every call); I can exclude calls marked spam if you prefer. ☐ (b) should Nabil press "1" for DoorDash's store-open check? Today it hangs up; pressing 1 would confirm you're open on DoorDash during open hours. Everything else is the same ask as before: the staging phone number (A67 #1).**)
**Previous update:** 2026-08-22 (evening) by Claude (**A69 — Five more fixes from the call analysis are built and on the STAGING copy; your live number is still untouched.** In order: (1) **no more dead air** — Nabil now speaks within 2.5 seconds of you finishing, whatever it is doing behind the scenes, says "still with you, one second" if the model stalls at 6 seconds, and retries a stalled hop at 8; (2) **a silent line is handled** — after a question, 8 seconds of silence gets one gentle nudge and 30 seconds gets a polite goodbye and hang-up (it never rings your phone for a silent caller any more); (3) **every call record is kept** — writes retry, get parked and re-sent if the site is slow, a placed order whose record was lost is stitched back onto its call, and the labels are honest ("Left mid-order" instead of "Question answered"); (4) **ordering logic** — the price is now quoted before the name is asked, a street the map matched to a DIFFERENT street is read back and confirmed instead of silently accepted, a country/city-level map hit is never treated as an address, "Large 3 Topping" with no toppings is asked about, and reservation dates like "today"/"Friday"/a past date are handled properly; (5) **callers asking about an order they placed ONLINE or in the app** — Nabil looks it up by their phone number (or the order number) and tells them the real status and ready time; DoorDash/Uber/Skip orders are pointed to that app; cancelling or changing goes to a person. It is ON for every store under Settings → Capabilities ("Check on orders already placed") and can be switched off. **Your part is unchanged: ☐ the staging phone number (A67 #1) so you can hear all of this before it goes live — every piece above rolls to your live number in one command once you have.**)
**Previous update:** 2026-08-22 (later) by Claude (**A68 — Your transfer setting is BUILT: every store now chooses how easily a caller reaches a person.** Nabil AI → Settings → Handoff has a new choice, *When a caller asks for a person*: **Transfer right away** (what every store has today — nothing changes anywhere until an owner picks otherwise), **Only if they insist** (the first ask gets your line and Nabil keeps helping; a second ask is put through), or **Never transfer** (Nabil handles it and takes messages). Under the last two you can write the exact sentence Nabil says — leave it blank and it says *"Our staff are busy preparing orders and can't take calls right now, but I can help with anything about the restaurant"* (in the caller's language, all 38). With *Offer to take a message* on, a caller who can't be put through is asked what to pass on; the message is saved with the call and **texted to your transfer number (or your alert phone)**. This is enforced in code on every call — pressing 0 and Nabil getting stuck go through the same rule — not by asking the AI nicely, and it was proven against the real model: the 'never' store deflected and still took a complete large-pepperoni order. **It is on the STAGING copy only** (your live number is untouched). **Your part: ☐ 1. still the staging phone number from A67 #1 — once I have it, pick your level on the staging line, call it, say "can I talk to someone?", and tell me whether the wording and the text message you receive feel right; then I promote it to the live line in one command. ☐ 2. Tell me which level YOUR store wants as its own setting.**)
**Previous update:** 2026-08-22 by Claude (**A67 — Nabil AI: the full day of real calls was scored, the plan is approved, and the "nothing touches the live line untested" lane is BUILT.** All 52 calls from Aug 21 were evaluated against the "experienced employee" bar: **every placed order was right (14/14 — cart = ticket, quoted = charged)**; the problems are elsewhere — dead air of 8 s or more on 29 % of calls, two callers stuck for minutes after asking for a person, callers asking about an order they placed online being told an invented status, the price refused until a name was given, and a third of the outcome labels on your dashboard being wrong. Your four decisions are recorded (keep the ambience pipeline and harden it on a test line; Opus 5 scores every call with Fable checking it weekly; order-status lookup for online/app/delivery orders when the number matches one customer; hotfixes first). Plus your two additions: **a per-store transfer setting — never / only if they insist / as soon as they ask — with your own "staff is busy preparing orders…" line**, and **a staging lane so no fix ever reaches the live number untested**: a second copy of Nabil (`nabil-voice-staging`) now exists, a phone number can be pointed at it from Superadmin › Nabil Phone Lines, every change goes there first, you hear it, and promotion/rollback is one command with the previous version kept. **What I need from you: ☐ 1. A STAGING PHONE NUMBER — buy one more Twilio number (≈US$1.15/mo, Twilio console → Phone Numbers → Buy, any Ontario number) and tell me the number; I'll attach it to your restaurant on the staging lane. Until then I can use the demo line (+1 289 670 5454) for routing tests only (it fakes orders and caps calls at 4 min). ☐ 2. Menu data confirmations (2 min): should the Caesar Salad default to Caesar dressing instead of asking "which dressing?"; is "Vegetable Lasagna Large" listed twice on purpose; what item is "the Luigi special" — these become aliases/defaults, not prompt text. ☐ 3. Optional, for audio-level analysis of real recordings: paste your Twilio Account SID + Auth Token into `.env.local` on this machine as `FFOS_TWILIO_ACCOUNT_SID` / `FFOS_TWILIO_AUTH_TOKEN` (they already exist on Vercel; I can't read them from there). Nothing else is needed from you — the Vercel setting for the staging lane is already in place.**)
