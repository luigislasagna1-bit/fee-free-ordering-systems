# -*- coding: utf-8 -*-
"""Merge the workflow-translated domain-switch keys (flat maps in
domain-i18n-out/<locale>.json) into src/messages/<locale>.json.
Validates each file's key set against the English source first."""
import json, os, sys

TMP = r"C:\Users\luigi\AppData\Local\Temp\claude\C--FeeFreeOrderingSystems\cf0c6e28-2af8-43ed-926e-395bdc1f884d\scratchpad"
EN = os.path.join(TMP, "domain-i18n", "_all-en.json")
OUT = os.path.join(TMP, "domain-i18n-out")
...
for loc in LOCALES:
    ...
    if not os.path.exists(p):
        errors.append(f"{loc}: missing"); continue
    try:
        m = json.load(open(p, encoding="utf-8"))
    except Exception as e:
        errors.append(f"{loc}: parse {e}"); continue
    if set(m.keys()) != en_keys:
        errors.append(f"{loc}: key mismatch"); continue
    maps[loc] = m
