# -*- coding: utf-8 -*-
"""Merge the translated pager range string into src/messages/<locale>.json."""
import json, os, sys
TMP = r"C:\Users\luigi\AppData\Local\Temp\claude\C--FeeFreeOrderingSystems\cf0c6e28-2af8-43ed-926e-395bdc1f884d\scratchpad"
EN = os.path.join(TMP, "pager-i18n", "_all-en.json")
OUT = os.path.join(TMP, "pager-i18n-out")
...
for loc in LOCALES:
    ...
    for k, v in m.items():
        for ph in ("{from}", "{to}", "{total}"):
            if ph not in v: errors.append(f"{loc}: missing placeholder {ph}")
    maps[loc] = m
if errors:
...
print(f"done — {len(maps)} locales merged")
