...
| **Fly.io** | Nabil AI voice service (`nabil-voice`, 1× shared-cpu-1x 512MB always-on in iad; deployed 2026-08-09, scaled from 2→1 machines same night) | ~US$5–7/mo est. + egress | Card on file (Luigi, 2026-08-09). ❓ confirm first invoice; scale-to-zero NOT possible (live calls hold a WebSocket). ⚠️ **Single machine = single point of failure AND a deploy drops live calls.** `fly scale count 2` roughly doubles this line (~+US$5–7/mo) and buys redundancy; connection-based concurrency limits are already in `fly.toml` so a second machine would be used the moment it exists — owner decision (Luigi, 2026-08-11) |
