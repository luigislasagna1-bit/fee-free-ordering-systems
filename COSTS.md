...
| **Nabil AI — per-call-minute CEILING (Luigi 2026-08-15): ≤ US$0.40/min ALL-IN** | Measured on the 5 most recent real calls (2026-08-13→15): model **16–23¢/min** (e.g. 86¢ / 268 s) + Twilio ConversationRelay 7¢ + inbound 0.85¢ + recording 0.3¢ ≈ **25–31¢/min today** ✅. ⚠️ Sonnet 5 list price applies from **2026-08-31** (~×1.5 on the model share ⇒ ~34–40¢/min) — the release gate (`npm run nabil:release`) now FAILS if the sim's model spend exceeds 30¢ per est. call-minute, and `npm run nabil:bench` decides thinking on/off by pass-rate then cost. Levers in order: `NABIL_THINKING=off` (fewer output tokens), compaction (fewer input tokens), 1 h cross-call prompt cache (menu prefix read, not re-written), Deepgram Flux (cheaper STT inside CR). | ceiling: 40¢/min | Track ¢/min per call in the admin (VoiceCall.costCents / durationSeconds) — TODO surface as a column |
| Anthropic API — **Nabil torture-test suite** (2026-08-15) | `npm run nabil:sim` drives the REAL agent against a menu snapshot: 25 critical + 10 injection scenarios ≈ **$8 per single pass**, ≈ **$12–25 for the 3× release gate** (`npm run nabil:release`); a 200-scenario adversarial sweep ≈ $40–60. Per simulated call ≈ $0.15–0.30 (a real call is ~$0.50–0.90). | **2026-08-15 build day: ≈US$110** (~250 simulated calls across 6 passes + 3 gate runs — the one-off cost of finding/fixing bugs before a caller heard them). Steady state ~$14 per release gate; $0 when idle — **on demand only, never in preflight** | Local key rotated 2026-08-15 (`.env.local`); credit ran dry mid-run twice ⇒ turn on auto-reload (A55 step 1) |
