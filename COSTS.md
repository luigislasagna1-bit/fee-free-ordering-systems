...
| **Fly.io** | Nabil AI voice service (`nabil-voice`, **2× shared-cpu-1x 512MB always-on in iad since 2026-08-16** — redundancy + rolling deploys; graceful SIGTERM drain shipped `213f76fc` so a deploy no longer drops calls) | ~US$10–14/mo est. + egress | Card on file (Luigi, 2026-08-09). ❓ confirm first invoice; scale-to-zero NOT possible (live calls hold a WebSocket). Measured 1.06 MB RSS per concurrent call (`npm run nabil:capacity`) — cap 25/machine kept for CPU + Anthropic rate-limit headroom, not memory. |
| ElevenLabs — **voice samples** (Nabil AI, 2026-08-11) | ⏸ **NOT ENABLED — no `ELEVENLABS_API_KEY` set.** Would power the "hear this voice" play button in Settings → Voice, which reads the store's OWN greeting in each candidate voice. Without it the picker still works; only the sample is missing, and the UI says so. | ~US$5/mo Starter would cover it comfortably: samples are ≤220 chars, cached per (voice, speed, greeting), so a store auditioning all 10 voices costs a few thousand characters ONCE | ⚠️ Owner decision — Luigi. Note the voices used on live CALLS are billed through Twilio's ConversationRelay, not here; this key is only for the in-dashboard preview |
| **Nabil AI — per-call-minute target (Luigi 2026-08-15): ≈ US$0.40/min ALL-IN, quality first** | Measured on the 5 most recent real calls (2026-08-13→15): model **16–23¢/min** (e.g. 86¢ / 268 s) + Twilio ConversationRelay 7¢ + inbound 0.85¢ + recording 0.3¢ ≈ **25–31¢/min today** ✅. ⚠️ Sonnet 5 list price applies from **2026-08-31** (~×1.5 on the model share ⇒ ~34–40¢/min). **Luigi, later 2026-08-15 (after his first live call): quality and reliability first, then latency, then cost — going a little over 40¢ is acceptable.** So the release gate (`npm run nabil:release`) now WARNS past 30¢ model share (≈40¢ all-in) and FAILS only past 40¢ model share (≈48¢ all-in); `npm run nabil:bench` picks the most ACCURATE config (adaptive / off / opus), ties broken on latency, then cost. Levers in order: bookkeeping-merge (one model round fewer on name/pickup turns — shipped), `NABIL_THINKING=off` (fewer output tokens), compaction, 1 h cross-call prompt cache, Deepgram Flux. | target ~40¢/min, hard stop ~48¢ | Track ¢/min per call in the admin (VoiceCall.costCents / durationSeconds) — TODO surface as a column |
| Anthropic API — **Nabil torture-test suite** (2026-08-15) | `npm run nabil:sim` drives the REAL agent against a menu snapshot: 25 critical + 10 injection scenarios ≈ **$8 per single pass**, ≈ **$12–25 for the 3× release gate** (`npm run nabil:release`); a 200-scenario adversarial sweep ≈ $40–60. Per simulated call ≈ $0.15–0.30 (a real call is ~$0.50–0.90). **Round 2 (2026-08-15 evening, Luigi: "no more $100 cycles")**: a FREE deterministic tier (370 engine scenarios, `npm test`, $0) now carries the volume; the paid tier is run as targeted `--ids` (~$3–6), one bench (~$10), ONE gate `--repeat 2` before deploy; `--budget-cents` hard-stops any run. Round-2 actual spend ≈ **US$70** (targeted runs $6, three model benches $26, two full gates $34, re-runs $4) — over the $30 plan because the Sonnet-vs-Opus / thinking-on-vs-off question was settled with data (see OWNER-ACTIONS A55). | **2026-08-15 build day: ≈US$110** (~250 simulated calls across 6 passes + 3 gate runs — the one-off cost of finding/fixing bugs before a caller heard them). Steady state ~$14 per release gate (**2026-08-16 gate on `0ab9c73e`: $13.19 for one `--repeat 1` pass**, model 24.5¢/est-min); $0 when idle — **on demand only, never in preflight** | Local key rotated 2026-08-15 (`.env.local`); credit ran dry mid-run twice ⇒ turn on auto-reload (A55 step 1) |
...
## Update log
- **2026-08-22 v6** — New line item, same vendor (Anthropic API, pay-as-you-go, NOT the Claude Max plan): **Nabil LLM judge** (`/api/cron/voice-evaluate`, Opus 5 by default, `NABIL_JUDGE_MODEL` to change) scores every ENGAGED call after the free deterministic pass — ≈ US$0.05/call, so ≈ US$1.50–2.50/day per busy store; robocalls, dropped records and <2-turn calls are skipped. Tracked per call in `VoiceCallEvaluation.judgeCostCents`. Switch off = remove the cron entry in vercel.json.
