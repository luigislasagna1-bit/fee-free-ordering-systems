...
- **Phase 3 — Full i18n coverage** ⏳ in progress.
  ...
  - **3b admin-screen extraction (batched, AI-translated):** batch 1 ✅ (4 screens). batch 2+3 ✅ (24 screens). batch 4 ✅ (12 more client components: assign-coupon, inherited/revert menu, kitchen-workflow toggle, receipt renderer, brand reports, heatmap, email-verification banner, guided-setup pill, date-range picker, export menu, legal shell) — all 36 locales at parity (2825 keys). Translation now uses the robust temp-file + in-code-merge pattern (no clobber; lighter per-run load avoids the complex-script crash). ~24 server report pages remain (need getTranslations).
