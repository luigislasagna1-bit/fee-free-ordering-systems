...
- **Phase 3 — Full i18n coverage ✅ shipped 2026-06-03.**
  ...
  - **3b admin-screen extraction (batched, AI-translated) ✅ complete:** batch 1 (4 screens), batch 2+3 (24), batch 4 (12 client components), **batch 5 (26 server report/admin pages → `getTranslations`)**. All 66 admin screens now render in any of the 37 languages. Robust temp-file + in-code-merge pipeline (no clobber; for the larger batch-5 load the StructuredOutput schema was dropped so agents just write+verify their temp file, eliminating the at-scale failures). Final audit: **all 36 non-English locales at full parity with en.json — 3,258 keys, 0 missing / 0 extra / 0 placeholder-arg mismatches** (depth-aware ICU validation).
- **Phase 4 — Locale-aware dates + customer-page polish ✅ shipped 2026-06-03.** `formatDate(utils)` now accepts `{ locale, timeZone }` (backward-compatible). Customer hot-path stragglers internationalized: **AddressBook** (was 100% hardcoded English → new `addressBook` namespace, 16 keys), CheckoutModal placeholders/labels ("Free delivery", apt/buzzer/delivery-instructions, "Pick a time…", "Custom amount"), OrderingPageClient menu-search + sign-in. All 37 locales at parity.
  - **Phase 4b — the 3 deferred units ✅ all shipped 2026-06-03:**
    1. **Receipt timestamp timezone** — order times now print in the restaurant's IANA timezone instead of server UTC (a Rome 20:00 order was printing 18:00). GOLDEN-safe: changed ONLY the date string content via the existing module-scoped pattern (alongside currency); the EscPos byte machinery + `text()` sanitizer are untouched, and locale stays frozen at `en-US` so thermal output remains pure ASCII (localized month names would risk CP437 garbage). Threaded `timezone` through all 3 print routes + both renderers (receipt.ts thermal + receipt-lines.ts HTML/StarXpand).
