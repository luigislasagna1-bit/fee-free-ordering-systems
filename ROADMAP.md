...
- **Phase 4 — Locale-aware dates + customer-page polish ✅ shipped 2026-06-03.** `formatDate(utils)` now accepts `{ locale, timeZone }` (backward-compatible). Customer hot-path stragglers internationalized: **AddressBook** (was 100% hardcoded English → new `addressBook` namespace, 16 keys), CheckoutModal placeholders/labels ("Free delivery", apt/buzzer/delivery-instructions, "Pick a time…", "Custom amount"), OrderingPageClient menu-search + sign-in. All 37 locales at parity.
  - **Deliberately deferred (documented, not forgotten):** (1) `src/lib/receipt.ts` timestamp localization — untouched because it's inside the **GOLDEN printer pipeline Luigi locked**; do as an isolated, separately-verified change. (2) CheckoutModal closed/catering **scheduling-explanation block** (multi-branch rich text + `toLocaleString` date interpolation) — a self-contained rich-text i18n unit best wrapped whole rather than piecemeal. (3) Marketplace landing page (English-only — separate effort, see Marketplace notes).
