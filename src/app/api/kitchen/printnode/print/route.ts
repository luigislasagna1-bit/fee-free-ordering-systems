...
export async function POST(req: NextRequest) {
  try {
    ...
    const restaurant = await prisma.restaurant.findUnique({
      ...
      select: { name: true, address: true, city: true, state: true, zip: true, phone: true, email: true, defaultLanguage: true, currency: true, timezone: true, hoursFormat: true },
    });
    ...
    if (body.reservationId) {
      ...
      const buf = await buildReservationReceipt({
        ...
        hoursFormat: (receiptRestaurant as any).hoursFormat,
