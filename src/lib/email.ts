...
/**
 * "Text/email me the Order App link" — the restaurant owner requests the
 * download link on their preferred device from /admin/publishing (send-app-link
 * API). Availability-driven via APP_LINKS.kitchen: each store row renders only
 * while that listing is live. BOTH are live since 2026-08-15 (Play 2026-07-22,
 * App Store 2026-08-15), so the old "iPhone version coming soon" paragraph is
 * gone. English body (same convention as the driver-invite email).
 */
export async function sendKitchenAppLinkEmail(params: { to: string; restaurantName: string }) {
  ...
  const html = `<div style="font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:560px;margin:0 auto;color:#111827">
    <p style="font-size:15px">Here's the download link for the <strong>Fee Free Order App</strong> for ${name}.</p>
    <p style="font-size:15px;line-height:1.6">Install it on the phone or tablet you'll use to take orders — new orders ring instantly, even with the screen off, and receipts print over WiFi. It's free on iPhone, iPad and Android.</p>
  ...
  return send({
    to: params.to,
    subject: "Your Fee Free Order App download link",
    html,
  });
}
...
export async function sendMarketingEmail(params: {
  ...
  restaurantEmail?: string;
  restaurantPhone?: string;
  /** `transportSuppressed` = the TRANSPORT never attempted the send (no Resend
   *  key, or a non-prod run without ALLOW_DEV_EMAIL=1). Deliberately a separate
   *  field from `skipped: "suppressed"`, which means the opposite kind of thing
   *  — this recipient is on the do-not-email list and was correctly skipped.
   *  A caller that persists "we mailed this person" must treat
   *  transportSuppressed as "nothing happened, try again later", never as sent;
   *  that conflation is what recorded 15 phantom sends on 2026-08-01. */
}): Promise<{ sent: boolean; skipped?: "suppressed" | "stale" | "no_basis" | "too_soon"; success?: boolean; transportSuppressed?: boolean; error?: string }> {
  ...
  return { sent: !!result.success, success: result.success, transportSuppressed: result.suppressed, error: result.error };
}
