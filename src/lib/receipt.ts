...
export interface ReceiptOrder {
  ...
  driveDirection?: string | null;       // compass bearing store→customer, e.g. "N"
  notes?: string | null;
  subtotal: number;
  taxAmount: number;
  deliveryFee: number;
...
async function renderKitchenSection(
  ...
  switch (section.type) {
    ...
    // ASAP vs scheduled — own section so it can be toggled / styled /
    // repositioned. Skipped for a reservation (its time is in k_reservation).
    // Mirrors receipt-lines.ts. Luigi 2026-06-09.
    case "k_timing":
      if (order.reservation) break;
