...
- **CASL / privacy chokepoints — never bypass them (2026-08-04).** (1) EVERY marketing / automated / bulk email MUST go through `sendMarketingEmail()` in `src/lib/email.ts` — it enforces the unified `EmailSuppression` do-not-email list (`src/lib/suppression.ts`), a valid CASL consent basis (`src/lib/marketing-consent.ts`), and injects the Unsubscribe + Delete-my-data footer. Never call `resend` directly (an ESLint rule blocks importing it outside `email.ts`) and never add a marketing path keyed on a model with no consent field. (2) EVERY table/column that stores customer PII MUST be added to `PII_ERASURE_MAP` in `src/lib/data-erasure.ts` in the SAME change, or a "delete my data" request will silently leave that data behind. (3) A new imported-contacts path needs an attested `ConsentBasis` + the 24-month implied-consent window, same as the kickstarter import.
...
- ❌ Don't stage with `git add -A` / `git commit -a` — ALWAYS stage explicit paths. Multiple Claude sessions share this working tree, and blanket staging has twice swept a sibling session's WIP into an unrelated commit (d1cd3316 took another session's script; ff7d7edb pushed a half-built feature's lib file to prod — harmless only by luck, 2026-08-10).
- ❌ Don't add a new way for an order to become "accepted" without calling `dispatchAcceptedOrderSafe()` (src/lib/delivery-dispatch.ts). Auto-accept shipped without it and every delivery order silently bypassed ShipDay for 3 days (fixed cf88e72e, 2026-08-10). The dispatch-watchdog cron will rescue misses within ~10-20 min and log `RESCUED` — that log line means a trigger is broken; fix the trigger, don't rely on the watchdog.
