...
- [ ] **Reseller-reports FIX BACKLOG (completeness review 2026-07-01/02, Fabrizio).** A per-report code-verified pass split the open reports. **✅ COMPLETE + ready for Fabrizio to verify/close (send the verify email from the Reseller Reports UI):** Report/dashboard calculations (cmqsoloe6), Coupon BUG/HIGH (cmqtllluu), Which-dishes-discounted (cmqv33v2o), Promo dish-categories multi-menu (cmqv36jr0), Empty cart (cmqwkzm0c), **Promo "Get it Now" (cmqtmfp2n) — redundant bottom button removed 2026-07-02 (c7e938cf)**, **Delivery/pickup timeframes (cmqt99i8s) — ASAP-delivery estimate clarified at checkout, 38 locales, 2026-07-02 (c7e938cf)**. **🔧 STILL TO FIX — 2 open sub-items, COUPLED (both need per-service EXTRAORDINARY/special-day hours wired into `liveOpenStatus`/`nextOpenDate` + the CheckoutModal slot picker, which today only see the raw weekly `openingHours`):** (1) _Closing days / closed services_ (cmqp8l948): special OPEN hours for a single service (e.g. delivery 10:00–20:00 today) must take precedence in the checkout time-slot picker + per-service open-now status. (2) _Opening hours per service_ (cmqnm3hv0): once (1) feeds the special-day start time, add a distinct "Pickup will open TODAY at xx:xx" checkout message (new i18n string × 38) shown only when the start comes from a special/extraordinary schedule. Do (1) then (2). **🆕 STILL TO BUILD — the 5 NEW reports:** delivery time-slot windows (cmqqxerxs), house-number-after-street-name delivery address format (cmqsn52d2), table-reservation screen polish — clickable phone + show email + day/month (cmqwds5jt), invoices show issuing company + VAT + invoice number (cmr1ty0lc), save a payment method in Billing without enabling paid services + PayPal for monthly (cmr1u3qxm).
