...
- [ ] **Auto-geocode the restaurant address by default on /admin/profile (Luigi 2026-07-02).** Today the LOCATION section requires the owner to click "Geocode from address fields" to drop/refresh the map pin. Instead, for all restaurants (esp. new ones), the pin should AUTO-pull + geocode from the entered address by default (debounced on address entry), and only THEN let the owner manually drag/adjust the pin or override it. Keep the manual pin adjust available; the button becomes a fallback/refresh, not a required step. Applies to all new restaurants.

- [ ] **Reseller-reports FIX BACKLOG (completeness review 2026-07-01/02, Fabrizio).** A per-report code-verified pass split the open reports. **✅ COMPLETE + ready for Fabrizio to verify/close (send the verify email from the Reseller Reports UI):** Report/dashboard calculations (cmqsoloe6), Coupon BUG/HIGH (cmqtllluu), Which-dishes-discounted (cmqv33v2o), Promo dish-categories multi-menu (cmqv36jr0), Empty cart (cmqwkzm0c), **Promo "Get it Now" (cmqtmfp2n) — redundant bottom button removed 2026-07-02 (c7e938cf)**, **Delivery/pickup timeframes (cmqt99i8s) — ASAP-delivery estimate clarified at checkout, 38 locales, 2026-07-02 (c7e938cf)**. **✅ NOW ALSO DONE (2026-07-02, 70f6cd56) — the 2 coupled special-day-hours reports:** (1) _Closing days / closed services_ (cmqp8l948): special OPEN hours for a single service (e.g. delivery 10:00–20:00 today) now take precedence in the checkout time-slot picker + earliest slot + per-service open-now status. (2) _Opening hours per service_ (cmqnm3hv0): distinct "🕒 {Service} opens TODAY at {time} (special hours)" checkout message (new i18n string × 38), shown only when the start comes from a special/extraordinary schedule. Both moved to fully-addressed + follow-up posted; awaiting Fabrizio's Confirm. **🆕 IN PROGRESS — the 5 NEW reports + the coupon rework (autonomous batch, 2026-07-02):** Coupon HIGH (cmqtllluu — discount lost on submit + guest email-match + restaurant visibility + auto-email + per-service + combinable), delivery time-slot ranges (cmqqxerxs), house-number-after-street delivery address (cmqsn52d2), table-reservation screen polish — clickable phone + show email + localized day/month (cmqwds5jt), invoices show issuing company + VAT + invoice number (cmr1ty0lc — needs Luigi: source legal entity/VAT), save a payment method in Billing without active services (cmr1u3qxm — card-save via SetupIntent autonomous; PayPal-for-monthly needs Luigi decision).
