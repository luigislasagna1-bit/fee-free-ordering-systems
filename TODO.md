...
- [ ] **🟠 MONEY: cart PREVIEW ≠ CHARGE — align apply-promos with the orders engine inputs (audit 2026-07-02, pre-existing).** An adversarial trace of `src/app/api/public/apply-promos/route.ts` (preview) vs `src/app/api/orders/route.ts` (charge) — both call the SAME engine (`src/lib/promo-engine.ts`) but feed it different inputs in 4 spots, so the previewed discount can differ from the charged one. NONE are from this session (special-hours verified pricing-neutral). Fix carefully with END-TO-END order tests (money hot path). Priority order:
...
- [ ] **Exclude gift cards from promo/coupon discounts (Luigi 2026-07-01, money-leak).** Noticed during coupon testing: a $10 coupon applied to a $10 Gift Card item → gift card bought for $0 (just tip). A customer could mint free store credit (buy gift cards free with coupons, then spend them). Most platforms exclude gift-card SKUs from all promo/coupon discounts. Fix: flag gift-card menu items (or a "not discountable" item flag) and have the promo engine skip them when computing eligible subtotal / per-item discounts, in BOTH apply-promos (preview) and orders (charge). Verify minimumOrder + free-item + %-off + fixed-cart all ignore gift-card lines. Medium priority (real money, but niche until gift cards are promoted).
