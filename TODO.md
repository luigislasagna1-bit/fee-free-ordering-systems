...
- [ ] **Order page — category headers as HERO banners with the name overlaid (Luigi 2026-06-30). SPEC LOCKED, build after B5.** The customer ordering page (`src/app/order/[slug]/OrderingPageClient.tsx` ~L280–365) renders each menu category as a small square thumbnail + text heading + chevron — Luigi says it looks weak on mobile AND desktop. Luigi reviewed mockups and picked **Option A — "banner band"** (compact hero, NOT the tall hero). Build spec:
