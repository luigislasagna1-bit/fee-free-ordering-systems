...
- [ ] **Reseller-reports FIX BACKLOG (completeness review 2026-07-01, Fabrizio).** A per-report code-verified pass split the open reports. **✅ COMPLETE + ready for Fabrizio to verify/close (send the verify email from the Reseller Reports UI):** Report/dashboard calculations (cmqsoloe6), Coupon BUG/HIGH (cmqtllluu), Which-dishes-discounted (cmqv33v2o), Promo dish-categories multi-menu (cmqv36jr0), Empty cart (cmqwkzm0c). **🔧 STILL TO FIX — open sub-items on IN_TESTING reports (Fabrizio follow-ups):** (1) _Opening hours per service_ (cmqnm3hv0): checkout needs a distinct "Pickup will open TODAY at xx:xx" message when the start time comes from an EXTRAORDINARY/special-day schedule (add a special-day branch to scheduleReason + a new i18n string × 38). (2) _Closing days / closed services_ (cmqp8l948): EXTRAORDINARY OPEN hours for a single service must take precedence in the checkout time-slot picker + feed the per-service open-now status — today the picker only offers the normal weekly slots (CheckoutModal slot enumeration gets raw weekly openingHours, no holiday/custom_hours). (3) _Delivery/pickup timeframes_ (cmqt99i8s): clarify at checkout that "about X min" is an estimate, not a delivery-arrival promise (copy + i18n × 38). (4) _Promo "Get it Now"_ (cmqtmfp2n): remove/rework the now-redundant big green bottom button (every product has its own Add/Customize now). **🆕 STILL TO BUILD — the 5 NEW reports:** delivery time-slot windows (cmqqxerxs), house-number-after-street-name delivery address format (cmqsn52d2), table-reservation screen polish — clickable phone + show email + day/month (cmqwds5jt), invoices show issuing company + VAT + invoice number (cmr1ty0lc), save a payment method in Billing without enabling paid services + PayPal for monthly (cmr1u3qxm).
